export const mockData = [
  {
    id: 1,
    itemName: 'Apartment',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 23000,
    type: 'rent',
    displayImage: 'https://assets-news.housing.com/news/wp-content/uploads/2022/02/27121904/featured-compressed-67.jpg',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 2,
    itemName: 'House',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 4500,
    type: 'rent',
    displayImage:
      'https://3.bp.blogspot.com/-86Ak0IGhCbA/WN-3wq8xGFI/AAAAAAAAAJE/B7IM3xX1MuooPZq9IdsLCuIqe7qKzgeGwCLcB/s1600/6.jpg',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 3,
    itemName: 'Vila',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 3400,
    type: 'rent',
    displayImage:
      'https://1.bp.blogspot.com/-h9VZcPSqpuk/WN-3rkpWA8I/AAAAAAAAAIw/asE3C0WkZxYOy2575DYH_pPTzF3IHIrvACLcB/s1600/29.jpg',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 4,
    itemName: 'Land',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 5000,
    type: 'rent',
    displayImage:
      'https://2.bp.blogspot.com/-Q7-2jXFkA2c/WN-3hLSPbEI/AAAAAAAAAIA/GV4N7tQKgqMgvrQN6nKQhSFW5URah2gwgCLcB/s1600/18.jpg',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 5,
    itemName: 'Land',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 4600,
    type: 'rent',
    displayImage:
      'https://1.bp.blogspot.com/-_7OjEYkOD7c/WOAHbnMnSfI/AAAAAAAAAPQ/OefIX3X1aakiFjkh9zHPcKGfpupQ7NyWgCLcB/s1600/94.jpg',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 6,
    itemName: 'Hotle',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 6000,
    type: 'rent',
    displayImage: 'https://photos.ips-cambodia.com/photos/2107171332b8a2ab-White-Terrace-01.tab.jpg',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 7,
    itemName: 'Hotle',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 3000,
    type: 'rent',
    displayImage:
      'https://cf.bstatic.com/xdata/images/hotel/max1024x768/134475469.jpg?k=986e0385365fa9e17ef6497e2fb7d5e16552358ad343c4ad8fc35b29802eacac&o=&hp=1',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 8,
    itemName: 'Hotle',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 7000,
    type: 'rent',
    displayImage:
      'https://cf.bstatic.com/xdata/images/hotel/max1024x768/412234542.jpg?k=0eeb9429ff1eb96503723260bfb93218f7024d95d233bab4ed6107c9356975c8&o=&hp=1',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 9,
    itemName: 'cando',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 8000,
    type: 'rent',
    displayImage: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRsYChhAlcbmD2MapfoqQrIaNMjFBSZjExu0A&usqp=CAU',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 10,
    itemName: 'Cando',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 8000,
    type: 'rent',
    displayImage: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTP3n9BYY2RwRCAcVXttSjNp07Cky2fla0pcw&usqp=CAU',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 11,
    itemName: 'House',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 10000,
    type: 'rent',
    displayImage: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSIC2VZnE0g4j19IXGG4ivqXnRaBzYWdF4DNQ&usqp=CAU',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },

  {
    id: 12,
    itemName: 'House',
    itemDescription:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate consequuntur quidem voluptate voluptas, molestiae odit optio, excepturi porro est, dolorum totam doloremque. Similique quaerat natus eveniet dolore voluptas! Placeat, voluptate perferendis atque, nisi nihil autem laboriosam porro quam vitae aliquid similique magnam numquam itaque minus illo corporis maxime vero fugit',
    price: 20000,
    type: 'rent',
    displayImage: 'https://i.pinimg.com/originals/c7/a2/ff/c7a2ffe102cafee75f625b7e86734318.jpg',
    allImages: [''],
    details: {
      location: ['Toul Kork', 'Phnom Penh'],
      size: 30,
      bed: 2,
      bath: 2,
    },
    contactPerson: {
      profileImg: 'https://daisyui.com/images/stock/photo-1534528741775-53994a69daeb.jpg',
      name: 'Sery Vathana',
      phoneNumber: '0232398324',
    },
    customerReviews: [
      {
        name: 'Sery Vathana',
        message:
          'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fuga quidem nam mollitia natus quae tenetur, quod obcaecati suscipit totam sequi rem ut reprehenderit repellendus quibusdam assumenda ducimus, quis nisi quam!',
        rating: 5,
      },
    ],
  },
];
